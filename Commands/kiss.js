...
exports.callback = async(client, message, args) => {
  try {
    ...
    if(!user) {
     return message.reply(`${cfg.erroremoji} | Bạn phải @ đến một ai đó!`)}
    if(user.id === message.author.id) {
      return message.reply(`${cfg.erroremoji} | Đừng có tự sướng thế chứ ${cfg.FunEmoji}`)}  
    let slappers = ["https://c.tenor.com/hK8IUmweJWAAAAAC/kiss-me-%D0%BB%D1%8E%D0%B1%D0%BB%D1%8E.gif",
